...
async function startServer() {
  ...
  const apolloServer = new ApolloServer({ schema: await buildGraphQLSchema(), context: { config } });
  ...
  server.listen(PORT, () => {
    console.log(`Servidor GraphQL corriendo en http://localhost:${PORT}${apolloServer.graphqlPath}`);
  });
}
